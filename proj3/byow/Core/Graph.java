...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        while (mst.getAllEdges().size() != getAllVertices().size() - 1) {
            Edge e = fringe.poll();
            if (visited.contains(e.getDest())) {
                continue;
            }
            visited.add(e.getDest());
            if (!e.equals(new Edge(start, start, 0))) {
                mst.addEdge(e);
            }
            for (Edge w : getEdges(e.getDest())) {
                if (!mst.containsVertex(w.getDest())) {
                    fringe.add(w);
                }
            }
        }
    ...
    public Graph kruskals() {
        Graph mst = new Graph();
        UnionFind sets = new UnionFind(getAllVertices().size());
        for (int vertex : getAllVertices()) {
            mst.addVertex(vertex);
        }
        for (Edge e: getAllEdges()) {
            if (!sets.connected(e.getSource(), e.getDest())) {
                sets.union(e.getSource(), e.getDest());
                mst.addEdge(e);
            }
        }
        return mst;
